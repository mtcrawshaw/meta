...
def test_update_point_avg() -> None:
    ...
    ema_alpha = 0.9
    ema_threshold = 1
    metric = Metric(point_avg=True, ema_alpha=ema_alpha, ema_threshold=ema_threshold)
    ...
    # Verify computed values.
    assert np.allclose(metric.history, [0.0, 2.0, -0.5])
    assert np.allclose(metric.mean, [0.0, 0.2, 0.13])
    assert np.allclose(metric.stdev, [0.0, 0.6, sqrt(0.3681)])
