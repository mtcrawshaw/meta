...
def test_save_load() -> None:
    ...
    # Compare metrics.
    assert first_metrics.state_vars == second_metrics.state_vars
    for metric_name in first_metrics.state_vars:
        ...
        # Check that best in first run is not better than best in second run.
        if first_metric.maximize:
            assert first_metric.best <= second_metric.best
        else:
            assert first_metric.best >= second_metric.best

        for key in ["history", "mean", "stdev"]:
            first_val = getattr(first_metric, key)
            second_val = getattr(second_metric, key)
            n = len(first_val)
            assert first_val[:n] == second_val[:n]
