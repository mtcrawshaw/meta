...
def test_mean_ema():
    """ Test computation of EMA in Running Mean. """
    ...
    for i in range(EMA_THRESHOLD, len(data)):
        mean.update(data[i])
        expected_mean = expected_mean * EMA_ALPHA + data[i] * (1. - EMA_ALPHA)
        assert torch.allclose(mean.mean, expected_mean)


def test_mean_rand_ema():
    """ Test computation of EMA in Running Mean, with random values. """

    # Set up case.
    shape = (10, 10)
    data = torch.rand(EMA_THRESHOLD + 200)
...
def test_mean_std_arithmetic():
    ...
    for i in range(len(data)):
        ...
        if i > 0:
            assert torch.allclose(stats.stdev, torch.std(data[0:i+1], dim=0, unbiased=False))
...
def test_mean_std_rand_arithmetic():
    ...
    for i in range(len(data)):
        ...
        assert torch.allclose(stats.mean, torch.mean(data[0:i+1], dim=0))
        if i > 0:
            assert torch.allclose(stats.stdev, torch.std(data[0:i+1], dim=0, unbiased=False), atol=TOL)
...
def test_mean_std_ema():
    ...
    for i in range(EMA_THRESHOLD, len(data)):
        stats.update(data[i])
        expected_mean = expected_mean * EMA_ALPHA + data[i] * (1. - EMA_ALPHA)
        expected_square_mean = expected_square_mean * EMA_ALPHA + data[i] ** 2 * (1. - EMA_ALPHA)
